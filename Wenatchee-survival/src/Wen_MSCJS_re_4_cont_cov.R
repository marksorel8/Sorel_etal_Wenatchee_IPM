...
make_dat<-function(mark_file_CH=mark_file_CH,sites=c("LWe_J","McN_J","JDD_J","Bon_J","Est_J","Bon_A","McN_A","PRa_A","RIs_A","Tum_A"),start_year=2006, end_year=2017,cont_cov,length_bin=5,doy_bin=10,inc_unk=FALSE,exc_unk=FALSE){
  ...
  mean_DOY<-NA
  sd_DOY<-NA
   # sites=c("LWe_J","McN_J","Bon_J","Bon_A","McN_A","Tum_A");start_year=2006; end_year=2017;cont_cov=c();length_bin=5;doy_bin=10;inc_unk=FALSE;inc_unk=FALSE;exc_unk=FALSE#"rel_DOY_bin"
  ...
  if(is.null(all_of(cont_cov))){
    ...
    dat_out<- mark_file_CH %>%  
      ...
      mutate(length_bin=ceiling(Length.mm/length_bin)*length_bin-(length_bin/2),
             rel_DOY_bin=ceiling((`Mark.Day.Numberr`+ifelse(LH=="smolt",365,0))/doy_bin)*doy_bin-(doy_bin/2) ,
    ...
    #mean rel_DOY by LH
    mean_DOY<-dat_out %>% group_by(LH) %>% dplyr::summarize(mean_doy=mean(rel_DOY_bin))
    sd_DOY<- dat_out %>% group_by(LH) %>% dplyr::summarize(sd_doy=sd(rel_DOY_bin))
    
    dat_out<-left_join(dat_out,mean_DOY) %>% left_join(sd_DOY) %>%
      mutate(rel_DOY_bin=(rel_DOY_bin-mean_doy)/sd_doy)
  ...
  phi_pim_sim<-matrix(1,1,1)
    
    
    # inner_join(releases,Phi.design.dat %>% select(par.index,time,stratum,c("LH","stream","sea_Year_p",cont_cov))) %>% arrange(time,stratum) %>% # combine releases with design data
...
fit_wen_mscjs<-function(x,phi_formula, p_formula, psi_formula,doFit=TRUE,silent=FALSE,sd_rep=TRUE,sim_rand=1,REML=FALSE,hypersd=1,map_hypers=c(FALSE,FALSE),pen=c(1,1),start_par=NULL){
  ...
  if(doFit){ 
    ...
    # upper[(mod$par %>% names %>% substr(1,3))=="pen"]<-6
